        ...
        if (pw === repeatpw) {
          ...
          connection.query(sql2, (err, res) => {
            ...
            if (res[0]) {
              ...
              let sql = `INSERT INTO \`users\` (\`id\`, \`username\`, \`email\`, \`password\`, \`token\`, \`avatar\`, \`bio\`, \`created_at\`) VALUES (NULL, '${username}', '${email}', '${passwordhash.generate(
                pw
              )}', '${generateToken()}', '${avatar}', '', CURRENT_TIMESTAMP)`;
              try {
